...
## Purpose
Guide the frontend implementation of the Validation workspace using existing styles and assets.
...
## Files to Create
- `app/lab/validation/page.tsx` (Main workspace view)
- `components/validation/` (Domain-specific layout items if not present in MQDS)
...
## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/validation-mock.json`.
