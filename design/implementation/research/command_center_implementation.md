# Research Command Center Implementation Guide

## Design Source
- **Stitch Folder**: `design/stitch/current/research_command_center_1`
- **Source Files**: `code.html`, `screen.png`

## Purpose
Guide the frontend implementation of the Research Command Center workspace using existing styles and assets.
...
## Files to Create
- `app/lab/command-center/page.tsx` (Main workspace view)
- `components/command_center/` (Domain-specific layout items if not present in MQDS)

## MQDS Components
- Buttons, Sidebar, Page Headers, Status Pills, Tables, Input Fields.

## Routing
- **Route**: `/lab/command-center`

## Required APIs
- `GET `/api/scheduler/jobs` (Returns active and historic jobs)\nPOST `/api/scheduler/jobs/trigger` (Starts a training pipeline)`

## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/command_center-mock.json`.

## Future Production APIs
- FastAPI routes in `ml_service/api/` matching request/responses.

## Implementation Rules
1. **Pixel Perfect**: Reference `design/stitch/current/research_command_center_1/code.html` and `design/stitch/current/research_command_center_1/screen.png`.
