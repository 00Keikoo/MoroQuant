...
## Purpose
Guide the frontend implementation of the Calibration workspace using existing styles and assets.
...
## Files to Create
- `app/lab/calibration/page.tsx` (Main workspace view)
- `components/calibration/` (Domain-specific layout items if not present in MQDS)
...
## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/calibration-mock.json`.
