# Trades Implementation Guide

## Design Source
- **Stitch Folder**: `design/stitch/current/trading_dashboard_v2`
- **Source Files**: `code.html`, `screen.png`

## Purpose
Guide the frontend implementation of the Trades workspace using existing styles and assets.
...
## Files to Create
- `app/trading/trades/page.tsx` (Main workspace view)
- `components/trades/` (Domain-specific layout items if not present in MQDS)
...
## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/trades-mock.json`.

## Future Production APIs
- FastAPI routes in `ml_service/api/` matching request/responses.

## Implementation Rules
1. **Pixel Perfect**: Reference `design/stitch/current/trading_dashboard_v2/code.html` and `design/stitch/current/trading_dashboard_v2/screen.png`.
