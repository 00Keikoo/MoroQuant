...
## Purpose
Guide the frontend implementation of the Workers workspace using existing styles and assets.
...
## Files to Create
- `app/operations/workers/page.tsx` (Main workspace view)
- `components/workers/` (Domain-specific layout items if not present in MQDS)

## MQDS Components
- Buttons, Sidebar, Page Headers, Status Pills, Tables, Input Fields.

## Routing
- **Route**: `/operations/workers`

## Required APIs
- `GET `/api/workers/metrics` (Returns cluster CPU, GPU, memory, and status of running tasks)`

## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/workers-mock.json`.
