...
## Purpose
Guide the frontend implementation of the Monitoring workspace using existing styles and assets.
...
## Files to Create
- `app/operations/monitoring/page.tsx` (Main workspace view)
- `components/monitoring/` (Domain-specific layout items if not present in MQDS)
...
## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/monitoring-mock.json`.
