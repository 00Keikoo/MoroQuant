...
## Purpose
Guide the frontend implementation of the Scheduler workspace using existing styles and assets.
...
## Files to Create
- `app/operations/scheduler/page.tsx` (Main workspace view)
- `components/scheduler/` (Domain-specific layout items if not present in MQDS)
...
## Temporary Mock Data
- Shape should mirror output of API schema. Keep initial mock in `lib/mock-data/scheduler-mock.json`.
