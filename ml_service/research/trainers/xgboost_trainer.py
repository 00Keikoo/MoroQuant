...
class XGBoostTrainingMetrics(TrainingMetrics):
    """Immutable metrics subclass for XGBoost extending TrainingMetrics with classification statistics."""
    accuracy: float = 0.0
    precision: float = 0.0
    recall: float = 0.0
    f1: float = 0.0
...
class XGBoostTrainer(BaseTrainer):
    ...
    Provides real training and validation using the official xgboost library.
    """
    ...
    def __init__(self) -> None:
        ...
        self._metrics: Optional[XGBoostTrainingMetrics] = None
        self._artifact: Optional[ArtifactMetadata] = None
        self._model: Optional[xgboost.Booster] = None
        self._feature_cols: list[str] = []
        self._target_col: Optional[str] = None
        self._dtrain: Optional[xgboost.DMatrix] = None
        self._dval: Optional[xgboost.DMatrix] = None
    ...
    def validate(self, dataset: Any, features: Any, config: TrainerConfig, run: Any = None) -> None:
        ...
        # Ensure target column is a string if specified
    ...
    def prepare(self, dataset: Any, features: Any, config: TrainerConfig, run: Any = None) -> None:
        """
        Prepare the trainer by loading datasets, validating dimensions, and constructing DMatrix.
        """
        ...
        # 1. Read files and check fingerprints or fail immediately if files do not exist
        ...
        # 6. Split data for validation splits chronologically to preserve temporal causality
        ...
        # 7. Construct DMatrix
        self._dtrain = xgboost.DMatrix(X_train, label=y_train)
        self._dval = xgboost.DMatrix(X_val, label=y_val)

        self._is_prepared = True

    def train(self, dataset: Any, features: Any, config: TrainerConfig, run: Any = None) -> TrainingResult:
        """
        Execute the primary model fitting process using deterministic configuration.
        """
        ...
        # Enforce deterministic tree method if not specified
        if "tree_method" not in params:
            params["tree_method"] = "exact"

        # 2. Get training parameters
        train_params_dict = dict(config.training_parameters)
        num_boost_round = int(train_params_dict.get("epochs", train_params_dict.get("num_boost_round", 10)))
        early_stopping_rounds = train_params_dict.get("early_stopping_rounds")
        if early_stopping_rounds is not None:
            early_stopping_rounds = int(early_stopping_rounds)

        evals = [(self._dtrain, "train"), (self._dval, "validation")]
        ...
        # 4. Extract loss histories
        loss_history = ()
        val_loss_history = ()
        if "train" in evals_result and evals_result["train"]:
            metric_name = list(evals_result["train"].keys())[0]
            loss_history = tuple(float(x) for x in evals_result["train"][metric_name])
        if "validation" in evals_result and evals_result["validation"]:
            metric_name = list(evals_result["validation"].keys())[0]
            val_loss_history = tuple(float(x) for x in evals_result["validation"][metric_name])

        # 5. Evaluate on train/validation for initial metrics
        val_labels = self._dval.get_label()
        val_preds = self._model.predict(self._dval)

        # Binary thresholding for standard classification stats
        ...
        self._metrics = XGBoostTrainingMetrics(
        ...
        model_dump = self._model.get_dump()
        ...
        file_path = f"/storage/models/xgboost_{checksum[:16]}.json"
    ...
    def evaluate(self, dataset: Any) -> XGBoostTrainingMetrics:
        """
        ...
        deval = xgboost.DMatrix(X_eval)
        preds = self._model.predict(deval)
    ...
    def collect_metrics(self) -> XGBoostTrainingMetrics:
        """
    ...
    def generate_artifacts(self) -> dict:
        ...
        # Serialize model into memory structure
        model_dump = self._model.get_dump()
        importance = self._model.get_score(importance_type="weight")
