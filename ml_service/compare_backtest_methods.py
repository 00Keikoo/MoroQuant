...
def run_backtest_simulation(
    ...
    valid_conf = probas[valid_mask].max(axis=1) if valid_mask.any() else np.array([0.0])


    logger.info(
        f"Confidence stats: "
        f"mean={valid_conf.mean():.3f}, "
        f"median={np.median(valid_conf):.3f}, "
        f"max={valid_conf.max():.3f}, "
        f"p95={np.percentile(valid_conf,95):.3f}"
    )

    logger.info(
        f"Predictions >=60%: {(valid_conf >= 0.60).sum()}"
    )

    logger.info(
        f"Predictions >=70%: {(valid_conf >= 0.70).sum()}"
    )

    if confidence_threshold is not None:
        low_conf_mask = np.zeros(len(valid_mask), dtype=bool)

        if valid_mask.any():
            low_conf_mask[valid_mask] = (
                probas[valid_mask].max(axis=1) < confidence_threshold
            )
...
def run_comparison(symbol: str, timeframe: str, btc_df, spy_df):
	...
	if 'timestamp' not in df_bt.columns or 'close' not in df_bt.columns:
        ...
        predictions, probas, valid_mask = generate_walk_forward_predictions(
            df_bt, feature_cols, calibrator, regions
        )
        ...
        for threshold, threshold_name in confidence_thresholds:
            ...
            backtest_result = run_backtest_simulation(
                df_bt, predictions, probas, valid_mask,
