"""Tests for the real XGBoostTrainer implementation."""
...
from ml_service.research.trainers.xgboost_trainer import XGBoostTrainer, XGBoostTrainingMetrics
...
def temp_data_paths(tmp_path):
    ...
    # Create synthetic dataset DataFrame
    np.random.seed(42)
    n_rows = 100
    ...
    # Target correlation for valid metrics
...
def test_deterministic_seed(temp_data_paths, valid_config, sample_research_run):
    ...
    trainer_1 = XGBoostTrainer()
    res_1 = trainer_1.train(dataset, features, valid_config, sample_research_run)

    trainer_2 = XGBoostTrainer()
    res_2 = trainer_2.train(dataset, features, valid_config, sample_research_run)
...
def test_successful_training(temp_data_paths, valid_config, sample_research_run):
    ...
    assert isinstance(result.metrics, XGBoostTrainingMetrics)
...
def test_evaluation(temp_data_paths, valid_config, sample_research_run):
    ...
    assert isinstance(eval_metrics, XGBoostTrainingMetrics)
...
def test_artifact_generation(temp_data_paths, valid_config, sample_research_run):
    ...
    assert artifacts["metadata"]["model_type"] == "xgboost"
